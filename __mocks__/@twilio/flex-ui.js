...
class Actions {
    ...
    constructor() {
        this.invokeAction = jest.fn(async (name, payload) => {
            ...
            await Promise.all(
                lifecycle[1].map(async (listener) => await listener(payload))
            );
            await Promise.all(
                lifecycle[2].map(async (listener) => await listener(payload))
            );
        });
...
module.exports = {
    ...
    Tabs: class Tabs extends React.Component {
        render() {
            return <React.Fragment />;
        }
    },
    ...
    useFlexSelector: (equalityFn) => true,
};
