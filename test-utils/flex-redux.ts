...
import { AppState, reduxNamespace } from "../ui-src/src/flex-hooks/states/types";
...
export const setCustomReduxState = (
    appState: Partial<AppState[typeof reduxNamespace]>
) => {
    mergeWith(
        mockedReduxState,
        { [reduxNamespace]: appState },
        (objValue, srcValue, key, obj) => {
            if (srcValue === undefined) {
                unset(obj, key);
            }
